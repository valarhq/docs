...
Both surfaces also accept `stream: true` for token-level Server-Sent Events when you want to show output as it's generated — see the [streaming support matrix](/support#behavior-shared-across-every-api).
...
You can also keep the connection open with `stream: true` instead of `background: true` — streaming is accepted on both completion windows, so a `standard`-window call streams tokens live as the worker produces them and terminates at the same time the durable record reveals. Pick `background: true` when you want to poll or use webhooks; pick `stream: true` when the caller is happy to hold the connection and wants tokens as they arrive. The two are mutually exclusive.
