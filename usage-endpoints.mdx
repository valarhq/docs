...
<Note>
  The keys in `sla_mix` are [completion window](/completion-windows) names:
  `asap`, `priority`, `standard`, `flex`. You may also see legacy keys on older
  entries predating the canonical rate-card rollout — `15min` (now `priority`)
  and `24hr` (now `flex`).
</Note>
