...
Structured outputs are supported on Valar's APIs. The field you set differs per API, but the JSON Schema you pass is the same. Structured outputs work on both the Responses and Chat Completions APIs. The field you set differs per API, but the JSON Schema you pass is the same.
