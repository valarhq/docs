---
title: "Pricing"
description: "Per-token rates for ValarAI inference"
---
...
* ValarAI offers four completion windows: `standard`, `priority`, `flex`, and `asap`. Full details are on the [Completion Windows](/completion-windows) page.
  * Window availability varies by model. We continually add new models and broaden window coverage on existing ones as demand grows. If something you need isn't listed above, <a href="mailto:support@valarhq.ai">get in touch</a>.
* Caching happens automatically via prefix matching. To boost cache hit rates, you can optionally supply [`prompt_cache_key`](/api-reference/responses-api/create-a-response#body-prompt-cache-key) as a routing hint.
* Head to [Models](/models) for capabilities and further details on each supported model.
