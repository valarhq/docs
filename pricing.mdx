...
The rate you pay also depends on the completion window you request. ValarAI runs four windows - `now`, `standard`-  and faster scheduling carries a higher rate. The table lists every model with its windows priced out; not every model offers every window. [Completion Windows](/completion-windows) explains the trade-offs, and you can mix windows per request.
