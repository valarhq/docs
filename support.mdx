...
- **Completion windows steer scheduling and price.** Set `metadata.completion_window` to `"asap"` (the **Now** tier), `"standard"`, or `"flex"` (background only). See [Completion windows](/completion-windows) and [Pricing](/pricing).
