...
<Note>
  Valar offers two completion windows: **Now** (`asap`) for interactive turns, and **Standard** (`standard`) for cost-optimized work. Set one with the OpenAI-compatible provider's request metadata, for example `providerOptions: { 'valar': { completion_window: 'standard' } }`. See [Inference modes](/inference-modes#completion-windows) for details.
</Note>
