...
| `429 rate_limit_exceeded` \+ `Retry-After` | Organization request rate exceeded (60 req/min default) | Wait the `Retry-After` value, then retry. See [Limits and quotas](/limits) |
| `429 insufficient_quota` | Organization rolling 30-day spend cap reached (\$5,000 default) | Do not blind-retry — request a raise or wait for spend to age out. See [Limits and quotas](/limits) |
