...
<ParamField path="metadata.webhook_token" type="string">
  An optional shared secret. ValarAI sends its value as a Bearer token in the callback's `Authorization` header so your endpoint can confirm the request is genuine.
</ParamField>
